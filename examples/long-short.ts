...
const MINUTE = 60000;
enum SideType { BUY = 'buy', SELL = 'sell' }
enum PositionType { LONG = 'long', SHORT = 'short' }
...
class LongShort {
  ...
  constructor ({ keyId, secretKey, paper = true, bucketPct = 0.25 }: AlpacaConfig & { bucketPct?: number }) {
  ...
  async run (): Promise<void> {
    ...
    var spin = setInterval(async () => {
      ...
      try {
        let clock: Clock = await this.alpaca.getClock();
      ...
      if (this.timeToClose !== null && this.timeToClose < (MINUTE * INTERVAL)) {
        // Close all positions when 15 minutes til market close.
        console.log('Market closing soon. Closing positions.');

        try {
          let positions: Position[] = await this.alpaca.getPositions();
  ...
  async cancelExistingOrders (): Promise<void> {
    let orders: any[] = [];
    try {
  ...
  async rebalance (): Promise<void> {
    ...
    await Promise.all(positions.map(position => {
      return new Promise<void>(async (resolve, reject) => {
        let quantity = Math.abs(parseFloat(position.qty));
    ...
    try {
      ...
      let [longOrders, shortOrders] = await Promise.all([
        this.sendBatchOrder({
          quantity: this.qLong as number,
          stocks: this.long,
          side: SideType.BUY
        }),
        this.sendBatchOrder({
          quantity: this.qShort as number,
          stocks: this.short,
          side: SideType.SELL
        })
      ]);
  ...
  async getTotalPrice (stocks: string[] = []): Promise<number[]> {
    return Promise.all(stocks.map(stock => {
      return new Promise<number>(async (resolve) => {
        try {
          ...
          console.log(`Encountered error ${err.message} for ${JSON.stringify(stock)}.`);
          resolve(0);
        }
      });
    }));
  ...
  async submitOrder ({ quantity, stock, side }: Order): Promise<boolean> {
  ...
  async sendBatchOrder ({ quantity, stocks, side }: { quantity: number, stocks: string[], side: SideType }): Promise<{ incomplete: string[], executed: string[] }> {
    return new Promise<{ incomplete: string[], executed: string[] }>(async (resolve) => {
  ...
  async getPercentChanges (limit = 10): Promise<void> {
    await Promise.all(this.stockList.map(stock => {
      return new Promise<void>(async (resolve) => {
        try {
          // polygon and alpaca have different responses to keep backwards
          // compatibility, so we handle it a bit differently
...
async function generatorToArray(resp: AsyncIterable<any>): Promise<any[]> {
  let result: any[] = [];
